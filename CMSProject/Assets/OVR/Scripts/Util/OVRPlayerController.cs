...
/*		if (!Controller.isGrounded)
		...
		// Allows the right and left triggers to move the player along the Y axis
		if (OVRInput.Get (OVRInput.Button.SecondaryIndexTrigger) || Input.GetKey(KeyCode.Q)) {
			MoveThrottle += Acceleration * new Vector3 (0, transform.lossyScale.y * (JumpForce * 2), 0);
		} else if (OVRInput.Get (OVRInput.Button.PrimaryIndexTrigger) || Input.GetKey(KeyCode.E)) {
			MoveThrottle -= Damping * new Vector3 (0, transform.lossyScale.y * JumpForce, 0);
		} else {
			MoveThrottle = new Vector3 (0, 0, 0);
		}

        //Puts the player above ground
		if (/*OVRInput.GetDown(OVRInput.RawButton.RThumbstickDown)*/Input.GetKey(KeyCode.T))
        {
            transform.position = new Vector3(76, 12, 105);
			//transform.rotation = new  Quaternion(Vector3 (4, 185, -6));
        }

		//Puts the player back at the start position
		if (Input.GetKey (KeyCode.R)) {
			transform.position = new Vector3 (11,-119,-8);
		}
