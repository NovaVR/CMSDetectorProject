...
public class QuickCutsceneController : MonoBehaviour {
	...
	public void SetCameraPointReferences()
	{
		...
		{
			Debug.Log("No child transforms (camera points) found in '" + gameObject.name + "'");
		}
	...
	public void ActivateCutscene()
	{
		...
		foreach(MonoBehaviour m in disableWhileInCutscene)
		{
			...
			{				
				...
				Debug.Log("Cutscene script " + m + " was not set");
				EndCutscene();
			}
		}
	...
	IEnumerator PlayCutscene()
	{
		...
		{
			Debug.Log("Camera event time " + cutsceneEventKeyTime[currentEvent] + " did not have a value set.");
			EndCutscene();
		}
		...
		if(broadcastMessageChoice[currentEvent])
		{
			...
			{				
				...
				Debug.Log("No target/method name for BroadcastMessage set");
			}
		}
		...
		{		
			...
			Debug.Log("Camera point " + cutsceneCameraPoints[currentEvent+2] + " did not have a value set.");
			EndCutscene();
		}
		...
		if(currentRotationalSpeedSetting == CameraRotationSpeedOptions.FollowTarget)
		{
			...
			{
				Debug.Log("No smooth follow target set for event " + (currentEvent+1));
				EndCutscene();
			}
		}
